...
var statsApi = 'https://osm-stats-production-api.azurewebsites.net';
var tasksApi = 'https://tasking-manager-tm4-production-api.hotosm.org/api/v2';
...
function getProjects(projects) {
  ...
  projects.forEach(function (project, i) {
    var url = tasksApi + ('/projects/' + project + '/queries/summary/');
...
function makeProject(project, projectOrder) {
  var projDone = Math.round(project.percentMapped);
...
function onEachFeature(feature, layer) {
  ...
  var taskStatus = feature.properties.taskStatus;
  if (taskStatus === "READY") {
...
function addMap(projectId) {
  ...
  var endpoint = tasksApi + ('/projects/' + projectId + '/tasks/');
...
function generateUserUrl(userName) {
  var userUrl = 'http://www.missingmaps.org/users/#/' + userName;
...
function getUserActivityStats(hashtag) {
  ...
  var url = statsApi + '/top-users/' + hashtag;
...
function getGroupActivityStats(hashtags, primaryHashtag) {
  ...
  var hashtagsString = [primaryHashtag].concat(hashtags).join(',');
  var url = statsApi + '/group-summaries/' + hashtagsString;
  ...
  $.getJSON(url, function (hashtagData) {
    ...
    if ($.isEmptyObject(hashtagData)) {
      ...
      console.warn('WARNING >> None of the secondary hashtags contain any ' + 'metrics according to the Missing Maps endpoint at ' + statsApi + '/group-summaries/' + hashtagsString + '. The partner graphs will not be displayed.');
    } else {
      var primaryData = hashtagData[primaryHashtag];
      var primaryBuildingCount = primaryData.building_count_add + primaryData.building_count_mod;
      var primaryRoadCount = Math.round(primaryData.road_km_add + primaryData.road_km_mod);
      ...
      var _hashtags = Object.keys(hashtagData).filter(function (x) {
        return x !== primaryHashtag;
      });

      var totalSum = _hashtags.reduce(function (acc, ht) {
        var vals = hashtagData[ht];
        if (!$.isEmptyObject(vals)) {
          var sum = Math.round(vals.building_count_add + vals.building_count_mod + vals.road_km_add + vals.road_km_mod + vals.waterway_count_add + vals.poi_count_add);
      ...
      var bldngSum = _hashtags.reduce(function (acc, ht) {
        var vals = hashtagData[ht];
        if (!$.isEmptyObject(vals)) {
          var sum = vals.building_count_add + vals.building_count_mod;
          acc.push({ name: ht, decorate: generateHashtagUrl, value: sum });
        }
        return acc;
      }, []).sort(function (a, b) {
      ...
      var roadsSum = _hashtags.reduce(function (acc, ht) {
        var vals = hashtagData[ht];
        if (!$.isEmptyObject(vals)) {
          var sum = Math.round(vals.road_km_add + vals.road_km_mod);
          acc.push({ name: ht, decorate: generateHashtagUrl, value: sum });
        }
        return acc;
      }, []).sort(function (a, b) {
...
function Barchart(data, targetElement) {
  ...
  // Add the name labels
  bars.append('text').attr('class', 'Graph-Label-Name').attr('x', 5).attr('y', barHeight / 2).attr('dy', '.35em').text(function (d) {
    return d.name;
  }).html(function (d) {
    return d.decorate(d.name);
  }).style('fill', '#606161');
...
function getGroupActivityStatsSubHashtag(hashtags, primaryHashtag) {
  ...
  $.getJSON(url, function (hashtagData) {
    ...
    if ($.isEmptyObject(hashtagData)) {
      ...
      // with the difference that there does not need to be a 1:1 match
      // between input and output array length
      var _hashtags2 = Object.keys(hashtagData).filter(function (x) {
        return x !== primaryHashtag;
      });

      var totalSum = _hashtags2.reduce(function (acc, ht) {
        var vals = hashtagData[ht];
        if (!$.isEmptyObject(vals)) {
          var sum = Math.round(vals.building_count_add + vals.building_count_mod + vals.road_km_add + vals.road_km_mod + vals.waterway_count_add + vals.poi_count_add);
          acc.push({ name: ht, decorate: generateHashtagUrl, value: sum });
      ...
      // For each hashtag, sum the total building edits,
      // skipping over hashtags if there are no metrics
      var bldngSum = _hashtags2.reduce(function (acc, ht) {
        var vals = hashtagData[ht];
        if (!$.isEmptyObject(vals)) {
          var sum = vals.building_count_add + vals.building_count_mod;
          acc.push({ name: ht, decorate: generateHashtagUrl, value: sum });
      ...
      var subhashtagsBldngCount = statsSum(bldngSum);

      // For each hashtag, sum the total road kilometers edited,
      // skipping over hashtags if there are no metrics
      var roadsSum = _hashtags2.reduce(function (acc, ht) {
        var vals = hashtagData[ht];
        if (!$.isEmptyObject(vals)) {
          var sum = Math.round(vals.road_km_add + vals.road_km_mod);
          acc.push({ name: ht, decorate: generateHashtagUrl, value: sum });
      ...
      // Sum all the building edits for all the subhashtags combined
      var subhashtagsRoadsCount = statsSum(roadsSum);

      var usersSum = _hashtags2.reduce(function (acc, ht) {
        var vals = hashtagData[ht];
        if (!$.isEmptyObject(vals)) {
          var sum = vals.users;
          acc.push({ name: ht, decorate: generateHashtagUrl, value: sum });
      ...
      var usersTotal = statsSum(usersSum);

      var contributionsSum = _hashtags2.reduce(function (acc, ht) {
        var vals = hashtagData[ht];
        if (!$.isEmptyObject(vals)) {
          var sum = vals.edits;
          acc.push({ name: ht, decorate: generateHashtagUrl, value: sum });
      ...
      var editsTotal = statsSum(contributionsSum);
...
var mbToken = 'pk.eyJ1IjoiYW1lcmljYW5yZWRjcm9zcyIsImEiOiJjaXF0MGpqY3AwMDFmZnJqNndodTYxNjZiIn0.2erhHFDW7G5Cvauho3vLMw';
var mbBasemapUrl = 'https://api.mapbox.com/styles/v1/americanredcross/ckl71lzvm0xh717nn6j10rmyt/tiles/{z}/{x}/{y}';
