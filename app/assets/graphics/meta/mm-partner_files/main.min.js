...
function getPrimaryStats(primaryhash) {
  var url = 'http://osmstats.redcross.org/hashtags/' + primaryhash + '/users';
...
function makePlaceholderProject(projectId, projectOrder) {
  ...
  var projectDescriptionEl = $('ul li:nth-child(' + projectOrder + ') .HOT-Description p');
  var errorHtml = projectDescriptionEl[0].innerHTML.split(' ').slice(0, 24).join(' ') + '...';
...
function generateUserUrl(userName, userId) {
  var userUrl = 'http://www.missingmaps.org/users/#/' + userId;
...
function getUserActivityStats(hashtag) {
  ...
  var url = 'http://osmstats.redcross.org/top-users/' + hashtag;
...
function getGroupActivityStats(hashtags) {
  ...
  var url = 'http://osmstats.redcross.org/group-summaries/' + hashtagsString;
  ...
  $.getJSON(url, function (hashtagData) {
    ...
    if ($.isEmptyObject(hashtagData)) {
      ...
      var totalSum = hashtags.reduce(function (acc, ht) {
      ...
      var bldngSum = hashtags.reduce(function (acc, ht) {
        var vals = hashtagData[ht];
        if (!$.isEmptyObject(vals)) {
      ...
      // skipping over hashtags if there are no metrics
      var roadsSum = hashtags.reduce(function (acc, ht) {
        var vals = hashtagData[ht];
        if (!$.isEmptyObject(vals)) {
...
var mbToken = 'pk.eyJ1Ijoic3RhdGVvZnNhdGVsbGl0ZSIsImEiOiJlZTM5ODI5NGYw' + 'ZWM2MjRlZmEyNzEyMWRjZWJlY2FhZiJ9.omsA8QDSKggbxiJjumiA_w.';
